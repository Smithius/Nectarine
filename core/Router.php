...
class Router {
	...
	function __construct() {
		if (!CLI) {
			...
			$this->site = Conf::get('nc.site', HOST);
			$uri = explode('?', $_SERVER['REQUEST_URI']);
			$uri = trim(array_shift($uri), '/');
			$uriSurfix = trim(preg_replace('/'. HOST . '/', "", $this->site, 1), '/');
			$this->path = preg_replace('/'. $uriSurfix . '/', "", $uri, 1);
		}
	}
	...
	public function url($controller, array $args = array(), $escaped = true) {
		...
		return rtrim($this->getUrl() . '/' . $pattern, '/');
	}
	...
	public function getUrl($subdomain = null) {
		return 'http' . (empty($_SERVER['HTTPS']) ? '' : 's') . '://'
				. (is_null($subdomain) || $subdomain === false ? Conf::get('nc.site', HOST) : $subdomain . '.' . ltrim(Conf::get('nc.domain', HOST), '.'));
	}
