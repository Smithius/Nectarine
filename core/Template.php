...
class Template {
	...
	public function renderView(Response $response = NULL) {
		...
		$template = ($response->getPath() ? ABSPATH . $response->getPath() : $this->getTemplatePath());
		$this->twig->getLoader()->setPaths($template . '/');
	...
	public function fnUrl($route, $args = array(), $escaped = true) {
		...
		if ($route === 'libs')
			return $this->router->getUrl(Conf::get('nc.assets', null)) . '/core/libs/' . (is_string($args) ? $args : '');
	...
	public function getAsset($name, $type) {
		$file = glob(ABSPATH . CORE . 'assets/' . ( $this->template ? $this->template . '/' : '' ) . $type . '/' . $name . '_v*.' . $type);
		if (is_null($file))
			return '';
